...
# A = benchmark()
# A = nx.to_numpy_array(G)
A = np.loadtxt("/home/carlo/workspace/communityalg/data/Coactivation_matrix_bin.adj")[
    0:64, 0:64
]
...
def sample(pij, reparametrization, sigmoid_slope=200):
    ...
    if reparametrization is "sigmoid":
        ...
        Amodel = tfp.distributions.RelaxedBernoulli(
            temperature=1 / sigmoid_slope, probs=pij
        ).sample()
        Amodel = tf.linalg.band_part(
            Amodel, 0, -1
        )  # like np.triu(A) for batched matrices
...
with tf.device("/device:CPU:0"):
    ...
    if method is "ADAM":
        ...
        exp_decayed_learning_rate = tf.train.exponential_decay(
            learning_rate=eta,
    ...
    with tf.Session() as sess:
        ...
        for b in beta_range:
            ...
            while True:
                tfvals = sess.run(
                    [train, exp_decayed_learning_rate, rel_entropy], feed_dict=feed_dict
                )
                iteration += 1
                ...
                if (iteration % refresh_frames) == 0:  # check every 100 iteration
                    res_slope, res_intercept = np.polyfit(
                        range(refresh_frames),
                        collect_dict["rel_entropy"][-refresh_frames:],
                        1,
                    )
                    if np.abs(res_slope) < 1e-4:
                        ...
                        def draw_fig():
                            ...
                            for i in range(batch_size):
                                ...
                                plt.semilogx(
                                    np.logspace(-4, 4, 50),
                                    nq.batch_compute_vonneumann_entropy(
                                        L0, np.logspace(-4, 4, 50)
                                    ),
                                    "r-",
                                    alpha=0.5,
                                )
                            ...
                            plt.semilogx(
                                b, nq.compute_vonneumann_entropy(L=L, beta=b), "ko"
                            )
                            plt.semilogx(
                                1, nq.compute_vonneumann_entropy(L=L, beta=1), "k*"
                            )
