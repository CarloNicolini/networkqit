...
with tf.device("/device:CPU:0"):
    ...
    x0 = tf.random_uniform(
        shape=[batch_size, N], minval=0.1, maxval=5, dtype=tf.float32
    )
            ...
            for epoch in tqdm(range(nepochs), desc="beta=%g" % b):
                ...
                if (epoch % refresh_frames) == 0:
                    ...
                    def draw_fig():
                        ...
                        plt.subplot(4, 2, 7)
                        plt.hist(
                            sess.run(lm, feed_dict=feed_dict).flatten(),
                            N,
                            density=True,
                            alpha=0.7,
                        )
