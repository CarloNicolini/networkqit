...
class UECM3(GraphModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.args_mapping = ["x_" + str(i) for i in range(0, kwargs["N"])] + [
            "y_" + str(i) for i in range(0, kwargs["N"])
        ]
    ...
    def loglikelihood(self, G, theta):
        ...
        loglike = (
            (k * np.log(x)).sum()
            + (s * np.log(y)).sum()
            + ilessjsum(np.log((1 - yiyj) / (1 - yiyj + xixj * yiyj)))
        )
        return loglike

    def saddle_point(
        self, G, theta
    ):  # equations (9,10) of 10.1088/1367-2630/16/4/043022
        k = (G > 0).sum(axis=0)
        pij = self.expected_adjacency(theta)
        avgk = pij.sum(axis=0) - np.diag(pij)
        w = G.sum(axis=0)
...
class CWTERG(GraphModel):
    ...
    def expected_weighted_adjacency(self, theta, expand_to_adj=False):
        x, y = theta[0], theta[1]
        t = self.threshold
        w = (x * (y ** t) * (1 - t * np.log(y))) / (
            (-x * (y ** t) + t * np.log(y)) * np.log(y)
        )
        if expand_to_adj:
    ...
    def fit(self, G, x0=None, **opt_kwargs):
        ...
        Wtotstar = G.sum() / 2
        if x0 is None:
            x0 = np.array([Lstar, Wtotstar]).reshape(
                [
                    2,
                ]
            )
...
class CWTCM(GraphModel):
    ...
    def expected_weighted_adjacency(self, theta):
        ...
        t = self.threshold
        yiyj = np.outer(y, y)
        wij = self.expected_adjacency(theta) * (
            (t * np.log(yiyj) - 1.0) / (np.log(yiyj))
        )
        return wij
    ...
    def loglikelihood(self, G, theta):
        ...
        loglike = (self._s * np.log(y)).sum() + ilessjsum(
            np.log(-np.log(yiyj) / ((yiyj ** t) - t * (np.log(yiyj))))
        )
        return loglike
...
class CWTECM(GraphModel):
    ...
    def loglikelihood(self, G, theta):
        ...
        if not hasattr(self, "_k"):
            ...
            self._s = G.sum(axis=0)
        loglike = (self._s * np.log(y) + self._k * np.log(x)).sum() + ilessjsum(
            np.log(-np.log(yiyj) / (xixj * (yiyj ** t) - t * (np.log(yiyj))))
        )
        return loglike
...
class CWTECM2(GraphModel):
    ...
    def saddle_point(self, G, theta):
        ...
        t = self.threshold
        pij = (
            self.expected_adjacency(theta) / t
        )  # because otherwise these are not probabilities
        avgk = pij.sum(axis=0) - np.diag(pij)
        w = G.sum(axis=0)
        wij = self.expected_weighted_adjacency(theta)
        avgw = wij.sum(axis=0) - np.diag(wij)
    ...
    def loglikelihood(self, G, theta):
        ...
        aij = G >= t
        wij = G
        loglike1 = (t * np.log(xixj) + wij * np.log(yiyj)) * aij + np.log(
            -np.log(yiyj) / ((xixj * yiyj) ** t - t * np.log(yiyj))
        )
    ...
    def sample_adjacency(self, theta, batch_size=1, with_grads=False, slope=500):
        ...
        pij = self.expected_adjacency(
            theta
        )  # gives numbers in [0,t] and not in [0,1] as typical models
        wij = self.expected_weighted_adjacency(theta)
        xixj = np.outer(x, x)
        yiyj = np.outer(y, y)
        if with_grads:
            A = expit(
                slope * (pij / t - rij)
            )  # it needs a gigantic slope to reduce error
            # to generate random weights, needs a second decorrelated random source
            rij = batched_symmetric_random(batch_size, self.N)
            W = -wij * np.log(rij) / pij
        else:
            ...
            W = trunc_exp_rv(
                low=t, scale=t * wij / pij, size=[batch_size, self.N, self.N]
            )
        W = np.triu(A * W, 1)
        W += np.transpose(W, axes=[0, 2, 1])
        return W
