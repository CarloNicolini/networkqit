...
def do_replacement(var, direction):
    ...
    if direction is 'forward':
        return var.replace(betai, -sp.log(yi)).replace(betaj, -sp.log(yj)).replace(alphai, -sp.log(xi)).replace(alphaj, -sp.log(xj)).replace(alphaij, -sp.log(xij)).replace(betaij, -sp.log(yij))
    elif direction is 'backward':
        return var.replace(yi, sp.exp(-betai)).replace(yj, sp.exp(-betaj)).replace(xi, sp.exp(-alphai)).replace(xj, sp.exp(-alphaj)).replace(xij, sp.exp(-alphaij)).replace(yij, sp.exp(-betaij))
