...
def density(L: np.array, beta_range: np.array):
    ...
    if n != m:
        raise RuntimeError(
            "Must input square or batched square laplacian [batch_size,n,n]"
        )
    ...
    if ndim == 3:
        ...
        def batched_eigvals_to_batched_diag(beta, v):
            return np.einsum(
                "lij,li->lij",
                (np.eye(n) + np.zeros([batch_size, n, n])),
                np.exp(-beta * v),
                optimize=True,
            )
        ...
        rho = np.asarray(
            [
                np.einsum(
                    "mij,mjk,mkl->mil",
                    Q,
                    batched_eigvals_to_batched_diag(beta, lambd),
                    np.transpose(Q, [0, 2, 1]),
                    optimize=True,
                )
                for beta in _beta_range
            ]
        )
    elif ndim == 2:
        rho = np.asarray(
            [
                np.linalg.multi_dot([Q, np.diag(np.exp(-beta * lambd)), Q.T])
                for beta in _beta_range
            ]
        )[:, None, :, :]
...
def relative_entropy_one_component(
    Lobs: np.array, Lmodel: np.array, beta_range: np.array
):
    """
    ...
    pade_expm = False
    if not pade_expm:
        lambd_obs, Q_obs = np.linalg.eigh(
            Lobs
        )  # this trick makes it possible to compute rho once
    ...
    if same_components:
        ...
        idx = np.argwhere(
            (lambd_model < 1e-12).astype(int).sum(axis=1)
            == num_obs_connected_components
        ).flatten()
        Lmodel = Lmodel[idx, :]
    ...
    for i, beta in enumerate(beta_range):
        if pade_expm:  # use the expm to compute von neumann density
            ...
            rho = np.linalg.multi_dot(
                [Q_obs, np.diag(np.exp(-beta * lambd_obs)), Q_obs.T]
            )
            rho /= np.trace(rho)
        Em[i] = np.mean(np.sum(np.sum(Lmodel * rho, axis=2), axis=1))  # average energy
        beta_Fm[i] = -np.mean(
            logsumexp(-beta * lambd_model, axis=1)
        )  # quenched log partition funtion
...
def relative_entropy(Lobs: np.array, Lmodel: np.array, beta_range: np.array):
    ...
    if ncomps > 1:  # disconnected graph, need to average over this function
        for c in np.unique(idx):
            if batches == 1:
                avg_dkl += relative_entropy_one_component(
                    Lobs=Lobs[np.ix_(idx == c, idx == c)],
...
class SpectralDivergence(object):
    ...
    def __init__(self, Lobs: np.array, Lmodel: np.array, beta: float, **kwargs):
        ...
        if kwargs.get("compute_js", False):
            ...
            self.jensen_shannon = entropy(l[l > 0]) - 0.5 * (
                entropy(eigvalsh(self.rho) + entropy(eigvalsh(self.sigma)))
            )
        self.options = kwargs
