...
from networkqit.graphtheory.models.MEModels import CWTCM
...
if __name__=='__main__':
    ...
    plot_mle(G,pij,wij,title='Loglikelihood LBFGS-B')
    plt.show()
    # TEST JACOBIAN
    ...
    # TEST SAMPLING
    Sd = (M.sample_adjacency(sol['x'], batch_size=1, with_grads=False)>t)[0,:,:]
    S = (M.sample_adjacency(sol['x'], batch_size=1, with_grads=False))[0,:,:]
    plot_mle(G, Sd.astype(float), S, title='Sampling')
    plt.show()
    # TEST SAMPLING WITH AVERAGING
    Sd = (M.sample_adjacency(sol['x'], batch_size=500, with_grads=False)>t).mean(axis=0)
    S = (M.sample_adjacency(sol['x'], batch_size=500, with_grads=False)).mean(axis=0)
    plot_mle(G, Sd.astype(float), S, title='Sampling averaging')
    plt.show()
