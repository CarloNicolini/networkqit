...
def plot_mle(G, pij, wij=None, **kwargs):
    ...
    (
        ...
        strength_p_value,
        strength_std_err,
    ) = fitting_stats(G.sum(axis=0), wij.sum(axis=0) - np.diag(wij))
    (
        degree_slope,
        degree_intercept,
        degree_r_value,
        ...
        (G > 0).sum(axis=0) - np.diag(G > 0), pij.sum(axis=0) - np.diag(pij)
    )
    ...
    ax[1, 2].plot(
        (G > 0).sum(axis=0) - np.diag(G > 0),
        pij.sum(axis=0) - np.diag(pij),
        "b.",
        MarkerSize=2,
    )
    ax[1, 2].plot(
        np.linspace(0, pij.sum(axis=0).max()),
        np.linspace(0, pij.sum(axis=0).max()),
        "r-",
    )
    ...
    ax[1, 2].text(
        0.05,
        0.95,
        "Fit $y \\sim \\alpha x + \\beta $:\n$\\alpha=$%.3f\n$\\beta=$%.3f\n$R^2=$%.3f\npval=%g"
        % (degree_slope, degree_intercept, degree_r_value, degree_p_value),
        bbox=props,
    ...
    if wij is not None:
        ...
        ax[0, 2].plot(
            np.linspace(0, wij.sum(axis=0).max()),
            np.linspace(0, wij.sum(axis=0).max()),
            "r-",
        )
        ...
        ax[0, 2].text(
            ...
            0.95,
            "Fit $y \\sim \\alpha x + \\beta $:\n$\\alpha=$%.3f\n$\\beta=$%.3f\n$R^2=$%.3f\npval=%g"
            % (strength_slope, strength_intercept, strength_r_value, strength_p_value),
            bbox=props,
...
def step_callback(A, beta, model, x, **kwargs):
    ...
    def drawfig1():
        ...
        plot_spectral_entropy(
            A, kwargs.get("beta_range", np.logspace(3, -3, 50)), color="r"
        )
        plot_spectral_entropy(
            model(x), kwargs.get("beta_range", np.logspace(3, -3, 50)), color="b"
        )
        ...
        plt.xticks(
            range(0, len(x)), kwargs.get("labels", range(0, len(x))), rotation=30
        )
        ...
        plt.plot(
            np.linspace(np.min(A.sum(axis=0)), np.max(A.sum(axis=0)), 2),
            np.linspace(np.min(A.sum(axis=0)), np.max(A.sum(axis=0)), 2),
            "r",
        )
