...
with tf.device("/device:CPU:0"):
    ...
    x0 = tf.random_uniform(
        shape=[batch_size, N], minval=0.0, maxval=1, dtype=tf.float32
    )
    ...
    with tf.Session() as sess:
        ...
        for b in beta_range:
            ...
            for epoch in tqdm(
                range(nepochs),
                desc="beta=%g eta=%g"
                % (b, sess.run(exp_decayed_learning_rate, feed_dict=feed_dict)),
            ):
                iteration += 1
                tfvals = sess.run(
                ...
                if (iteration % refresh_frames) == 0:
                    ...
                    def draw_fig():
                        ...
                        ki = A.sum(axis=0)
                        bounds, ixes = bct.grid_communities(
                            bct.community_louvain(A0)[0]
                        )
                        plt.imshow(A0)
