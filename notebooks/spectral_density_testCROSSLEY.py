...
def test2(matrix):
    ...
    allz, rho = compute_rho(np.linspace(0, 200, 100), ers, nr, matrix, eps=mp.mpf(1E-7), t0=[mp.mpc('0')] * len(nr), include_isolated=False)
...
def test3(matrix):
    ...
    W = loadmat('/home/carlo/workspace/BCT/data_and_demos/GroupAverage_rsfMRI_matrix.mat')['GroupAverage_rsfMRI']
    W = np.multiply(W,W>0)
    W =  W [0:250,0:250]
    memb, qual = bct.community_louvain(W)

    memb = nq.reindex_membership(memb)
    import networkqit as nq
    nr = [0]*len(np.unique(memb))
    for i in np.unique(memb):
        nr [i] = np.sum(memb==i)
    ers,ersnorm = nq.comm_mat(W,memb)
    eigs = scipy.linalg.eigvalsh(GL(W))
    #plt.hist(eigs,200)
    print(ers)
    allz, rho = compute_rho(np.linspace(2,100,10),ers,nr,'laplacian',eps=mp.mpf(1E-7), t0=[mp.mpc('0')] * len(nr), include_isolated=False)
    plt.plot(allz,np.abs(rho))
    plt.show()
