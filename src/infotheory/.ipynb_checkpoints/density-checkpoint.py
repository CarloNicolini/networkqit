...
#from networkqit.utils.matfun import expm, logm
#from mpmath import expm
from scipy.linalg import eigvalsh
from scipy.stats import entropy
from networkqit.graphtheory.matrices import graph_laplacian

# USE MPMATH HIGH PRECISION LIBRARIES FOR MATRIX EXPONENTIAL
# def expm(A):
#     from mpmath import expm
#     return np.array(expm(A,'pade').tolist(),dtype=np.float)

# def logm(A):
#     from mpmath import logm
#     return np.array(logm(A,'pade').tolist(),dtype=np.float)
...
class SpectralDivergence(object):
    def __init__(self, Lobs: np.array, Lmodel: np.array, beta: float, **kwargs):
        ...
        if self.fast_mode:  # prefer faster implementation based on eigenvalues
            self.lm = eigvalsh(Lmodel)
            self.lo = eigvalsh(Lobs)
            
            self.Zm = np.exp(-beta*self.lm).sum()
            self.Zo = np.exp(-beta*self.lo).sum()
        ...
        print('Zm=',self.Zm,'Zo=',self.Zo,'Em=',self.Em,'Eo=',self.Eo,'logLike=',self.loglike,'Srho=',self.entropy,'Dkl=',self.rel_entropy)
        #self.rel_entropy_slow = np.abs(np.real(np.trace(np.dot(compute_vonneuman_density(Lobs,beta),(logm(compute_vonneuman_density(Lobs,beta)) - logm(compute_vonneuman_density(Lmodel,beta)))))))
        #self.rel_entropy = self.rel_entropy_fast
        #self.rel_entropy_scipy = entropy(pk=np.exp(-beta*eigvalsh(Lobs)),qk=np.exp(-beta*eigvalsh(Lmodel)))
        #print(self.rel_entropy_fast,self.rel_entropy_slow)
